...
function initWaveCanvas() {
    ...
    let width = canvas.width = window.innerWidth;
    let height = canvas.height = document.querySelector('.contact').offsetHeight;
    
    window.addEventListener('resize', () => {
        width = canvas.width = window.innerWidth;
        height = canvas.height = document.querySelector('.contact').offsetHeight;
    });
    ...
    function animateWaves() {
        ...
        for(let i=0; i<width; i+=20) {
            const y = height/2 + Math.sin(i*0.01 + time) * 50 + Math.cos(i*0.02 + time*0.5) * 30;
